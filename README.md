...
64 projects, 59,492 lines of C#. Grouped by area; `Abstractions` packages hold contracts only and carry no runtime dependency.
...
| `KyrolusSous.ExceptionHandling.Runtime` | 34 | 988 | ExceptionHandling.Abstractions |

**Caching**

| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.Caching.Abstractions` | 29 | 1,061 | Compression |
| `KyrolusSous.Caching.MessagePack` | 2 | 88 | Caching.Abstractions |
| `KyrolusSous.Caching.Redis` | 23 | 3,698 | Caching.Abstractions |
...
| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.Compression` | 14 | 1,266 | - |
...
1. **Brotli (`BrotliCompressor`)**: Google's algorithm for maximum compression ratio (up to 25% better than Gzip).
...
5. **Gzip (`GzipCompressor`)**: Universal, universally compatible standard compression.
6. **Deflate (`DeflateCompressor`)**: Lightweight raw deflate compression.
...
builder.Services.AddKyrolusResponseCompression(options =>
{
    ...
    // Optional: override algorithm (default: auto-negotiated via Accept-Encoding)
    // options.WithPreferredAlgorithm(CompressionAlgorithm.Zstd);
});
```
