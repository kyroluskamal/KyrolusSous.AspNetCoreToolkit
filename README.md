...
137 projects, 125,943 lines of C#. Grouped by area; `Abstractions` packages hold contracts only and carry no runtime dependency.

**Localization**

| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.Localization.Abstractions` | 9 | 699 | - |
| `KyrolusSous.Localization.Json` | 5 | 550 | Localization.Abstractions |
| `KyrolusSous.Localization.StringLocalizer` | 3 | 96 | Localization.Abstractions |
...
| `KyrolusSous.Logging.Core` | 15 | 1,233 | Logging.Abstractions |
...
| `KyrolusSous.Mediator.Generator` | 4 | 2,719 | - |
| `KyrolusSous.Mediator.Reflection` | 4 | 672 | Mediator.Abstractions, Mediator.Runtime |
| `KyrolusSous.Mediator.Runtime` | 20 | 1,850 | Mediator.Abstractions |
...
| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.Compression.Abstractions` | 4 | 137 | - |
...
| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.Caching.Abstractions` | 31 | 2,102 | Compression.Abstractions |
| `KyrolusSous.Caching.MessagePack` | 3 | 138 | Caching.Abstractions |
| `KyrolusSous.Caching.Redis` | 24 | 4,528 | Caching.Abstractions |

**Mapping**

| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.Mapping.Abstractions` | 16 | 889 | - |
| `KyrolusSous.Mapping.Generator` | 4 | 609 | - |
| `KyrolusSous.Mapping.Runtime` | 12 | 1,930 | Mapping.Abstractions |
...
| `KyrolusSous.Repositories.Marten.Runtime` | 21 | 3,826 | ExceptionHandling.Runtime, Repositories.Marten.Abstractions |

**CQRS pipeline**

| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.CQRS.Abstractions` | 49 | 2,805 | Localization.Abstractions, Mediator.Abstractions |
| `KyrolusSous.CQRS.Caching` | 9 | 831 | CQRS.Abstractions, Caching.Abstractions, Mediator.Abstractions |
| `KyrolusSous.CQRS.EF` | 57 | 2,801 | CQRS.Abstractions, Mediator.Abstractions, Repositories.EF.Abstractions |
| `KyrolusSous.CQRS.Elasticsearch` | 26 | 917 | CQRS.Abstractions, Caching.Abstractions, Elasticsearch, Elasticsearch.Abstractions, Mediator.Abstractions |
| `KyrolusSous.CQRS.ExceptionHandling` | 4 | 91 | Mediator.Abstractions |
| `KyrolusSous.CQRS.Mapping` | 9 | 705 | CQRS.Abstractions, Mapping.Abstractions |
| `KyrolusSous.CQRS.Marten` | 54 | 2,382 | CQRS.Abstractions, Mediator.Abstractions, Repositories.Marten.Abstractions |
| `KyrolusSous.CQRS.Saga` | 11 | 1,015 | Mediator.Abstractions |
| `KyrolusSous.CQRS.Validation` | 6 | 274 | CQRS.Abstractions, Mediator.Abstractions, Validation.Abstractions |

**EndpointKit**

| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.EndpointKit.Core` | 53 | 4,837 | CQRS.Abstractions, Caching.Abstractions, Mediator.Abstractions, Validation.Abstractions |
| `KyrolusSous.EndpointKit.EF` | 24 | 5,150 | CQRS.EF, Caching.Abstractions, EndpointKit.Core, ExceptionHandling.ProblemDetails, ExceptionHandling.Runtime, Validation.Abstractions |
| `KyrolusSous.EndpointKit.Generator` | 1 | 1,229 | - |
| `KyrolusSous.EndpointKit.Marten` | 24 | 5,239 | CQRS.Marten, Caching.Abstractions, EndpointKit.Core, ExceptionHandling.ProblemDetails, ExceptionHandling.Runtime, Repositories.Marten.Abstractions, Validation.Abstractions |
...
| `KyrolusSous.Auth.MultiTenancy` | 12 | 389 | Auth.Abstractions |
...
| `KyrolusSous.Auth.TokenRevocation` | 7 | 400 | Auth.Abstractions, Caching.Abstractions |
...
| Project | Files | Lines | Depends on |
| --- | ---: | ---: | --- |
| `KyrolusSous.Elasticsearch` | 13 | 2,902 | Caching.Abstractions, Compression.Abstractions, Elasticsearch.Abstractions, Resilience.Abstractions |
| `KyrolusSous.Elasticsearch.Abstractions` | 7 | 1,196 | - |
...
| `KyrolusSous.FeatureManagement.Core` | 1 | 277 | FeatureManagement.Abstractions |
| `KyrolusSous.FeatureManagement.Redis` | 1 | 51 | FeatureManagement.Abstractions |
| `KyrolusSous.Gateway.Abstractions` | 27 | 2,900 | - |
| `KyrolusSous.Gateway.Yarp` | 20 | 3,779 | Auth.MultiTenancy, Auth.TokenRevocation, Gateway.Abstractions |
...
- `KyrolusSous.Compression.Abstractions`: Core interfaces (`IKyrolusCompressor`, `IKyrolusCompressionProvider`, `KyrolusCompressionAlgorithm`) with zero dependencies.
...
string compressedBase64 = largeJsonString.CompressString(KyrolusCompressionAlgorithm.Zstd);
string restoredString = compressedBase64.DecompressString(KyrolusCompressionAlgorithm.Zstd);
```
...
// In services:
public class ReportExporter(IKyrolusCompressionProvider compressionProvider)
{
    public async Task ExportReportAsync(Stream source, Stream destination)
    {
        var compressor = compressionProvider.GetCompressor(KyrolusCompressionAlgorithm.Brotli);
...
default in most templates) with the _delegate_ mapper instead of the dictionary one, so each lookup reads the
live value:
...
Lower values execute earlier (outer layers, closer to the caller); higher values execute later (inner layers, closer to the handler). This table is the single source of truth for every registered order value across the Mediator and CQRS libraries — when adding a new behavior, pick a free slot in the right band below and add a row here in the same change, so two behaviors never collide the way `KyrolusExceptionMappingBehavior` and `KyrolusRequestExceptionProcessorBehavior` once did at `-2000`.

|    Order    | Pipeline Behavior                                                                   | Library                              | Description                                                                                                                                      |
| :---------: | :---------------------------------------------------------------------------------- | :----------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------- |
| **`-2100`** | `KyrolusExceptionMappingBehavior`                                                   | `KyrolusSous.CQRS.ExceptionHandling` | Outermost layer: translates domain exceptions into structured responses or problem details.                                                      |
| **`-2050`** | `KyrolusAuditBehavior`                                                              | `KyrolusSous.CQRS.Abstractions`      | Captures audit records to `IKyrolusAuditSink`, including failures, before mapping strips them.                                                   |
| **`-2000`** | `KyrolusRequestExceptionProcessorBehavior`                                          | `KyrolusSous.Mediator.Runtime`       | Base Mediator exception-observer hook; must see the raw exception before it is mapped away.                                                      |
| **`-1050`** | `KyrolusAuthorizationBehavior`                                                      | `KyrolusSous.CQRS.Abstractions`      | Enforces authentication, roles, permissions, and named policies via `[KyrolusAuthorize]`.                                                        |
| **`-1040`** | `KyrolusTenantScopingBehavior`                                                      | `KyrolusSous.CQRS.Abstractions`      | Rejects a request naming a tenant other than the current user's (`IKyrolusTenantScopedRequest`).                                                 |
| **`-1000`** | `KyrolusRequestPreProcessorBehavior`                                                | `KyrolusSous.Mediator.Runtime`       | Base Mediator pre-processor hook, runs before any CQRS-specific behavior.                                                                        |
| **`-950`**  | `KyrolusValidationBehavior`                                                         | `KyrolusSous.CQRS.Validation`        | Aggregates multi-engine validators (FluentValidation + DataAnnotations); rejects before audit/idempotency/throttling consume their side effects. |
| **`-945`**  | `KyrolusBatchValidationBehavior`                                                    | `KyrolusSous.CQRS.Validation`        | Validates a batch command's `Items` via `IKyrolusValidationEngine.ValidateBatchAsync`, adjacent to (just inside) `KyrolusValidationBehavior`.    |
| **`-940`**  | `KyrolusPropertyAllowListBehavior`                                                  | `KyrolusSous.CQRS.Abstractions`      | Rejects mass-assignment of properties not declared in a request's allow-list.                                                                    |
| **`-900`**  | `KyrolusPerformanceAndTelemetryBehavior`                                            | `KyrolusSous.CQRS.Abstractions`      | OpenTelemetry distributed tracing spans, execution duration, and slow-request warnings.                                                          |
| **`-800`**  | `KyrolusIdempotencyBehavior`                                                        | `KyrolusSous.CQRS.Caching`           | Atomically claims the idempotency key before executing (`IKyrolusIdempotentCommand`).                                                            |
| **`-750`**  | `KyrolusThrottlingBehavior`                                                         | `KyrolusSous.CQRS.Abstractions`      | Concurrency limiting and semaphore-based backpressure per partition key.                                                                         |
| **`-660`**  | `KyrolusLivePushBehavior`                                                           | `KyrolusSous.CQRS.Abstractions`      | Real-time live update broadcasting to connected UI clients (SignalR / WebSockets).                                                               |
| **`-650`**  | `KyrolusDomainEventsDispatchBehavior` / `KyrolusMartenDomainEventsDispatchBehavior` | `KyrolusSous.CQRS.EF` / `.Marten`    | Drains and dispatches entity domain events via `IKyrolusMediatorPublisher`, only after the inner transaction behavior has committed.             |
| **`-600`**  | `KyrolusReadModelProjectionBehavior`                                                | `KyrolusSous.CQRS.Abstractions`      | Synchronizes polyglot read models (e.g. Elasticsearch/Redis) on successful command commits.                                                      |
| **`-560`**  | `KyrolusCommandCacheInvalidationBehavior`                                           | `KyrolusSous.CQRS.Caching`           | Invalidates cache entries/tags after a write, once the transaction behavior has committed.                                                       |
| **`-530`**  | `KyrolusEfTransactionBehavior` / `KyrolusMartenTransactionBehavior`                 | `KyrolusSous.CQRS.EF` / `.Marten`    | Atomic database transaction boundaries and execution strategies; commits before events dispatch.                                                 |
| **`-300`**  | `KyrolusQueryCachingBehavior`                                                       | `KyrolusSous.CQRS.Caching`           | Caches query responses under a tenant/user-scoped key, with pagination partitions.                                                               |
|   **`0`**   | `KyrolusStreamPassThroughBehavior`                                                  | `KyrolusSous.Mediator.Runtime`       | Base Mediator no-op passthrough for streaming requests.                                                                                          |
| **`1000`**  | `KyrolusRequestPostProcessorBehavior`                                               | `KyrolusSous.Mediator.Runtime`       | Base Mediator post-processor hook; innermost, runs closest to the handler's return.                                                              |

**Reserved bands** (pick an unused value inside the matching band for a new behavior; leave gaps for future insertions rather than renumbering existing ones):

- `-2100` to `-1900` — exception translation and audit (must wrap everything, including auth failures).
- `-1100` to `-1000` — identity/tenant gates (must run before anything reads user/tenant data).
- `-999` to `-900` — validation, mass-assignment, and telemetry (reject bad input before it is logged, throttled, or deduplicated).
- `-899` to `-600` — idempotency, throttling, transactions, domain events, and projections (execution-safety concerns, in dependency order: claim → limit → commit → dispatch → project).
- `-599` to `-300` — post-write invalidation and query caching.
- `0` and above — the Mediator's own base hooks, which every request passes through regardless of which CQRS libraries are installed.
...
- `KyrolusSous.EndpointKit.EF` (provider package)
  ...
  **What you get**
  ...
  - Cacheable flag propagated to CQRS (`IKyrolusCacheableRequest`).
...
{
  "request": {
    "filters": [{ "property": "Price", "operator": "lt", "value": "50" }],
    "useSplitQuery": false
  },
  "updates": {
    "Price": 55.5,
...
{
  "request": {
    "filters": [{ "property": "IsDeleted", "operator": "eq", "value": "true" }]
  },
  "cacheable": false
}
```
