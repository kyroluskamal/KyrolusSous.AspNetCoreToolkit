...
public sealed class KyrolusDataAnnotationsValidationGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var candidateClasses = context.SyntaxProvider.CreateSyntaxProvider(
            ...
            static (ctx, _) =>
            {
                ...
                if (ctx.SemanticModel.GetDeclaredSymbol(classDecl) is not INamedTypeSymbol symbol || symbol.IsAbstract || symbol.TypeParameters.Length > 0)
                    return null;
        ...
        out string fullValidatorName)
    {
        ...
        sb.AppendLine("        => ValidateCore(request, KyrolusValidationContext.Default, cancellationToken);");
        sb.AppendLine();
    ...
    private static string GenerateDiExtensions(List<(string TargetType, string ValidatorClassName, string FullValidatorName)> validators)
    {
        ...
        foreach (var (TargetType, ValidatorClassName, FullValidatorName) in validators.OrderBy(x => x.ValidatorClassName))
            sb.AppendLine($"        services.TryAddEnumerable(ServiceDescriptor.Scoped(typeof(IKyrolusRequestValidator<{TargetType}>), typeof({FullValidatorName})));");
