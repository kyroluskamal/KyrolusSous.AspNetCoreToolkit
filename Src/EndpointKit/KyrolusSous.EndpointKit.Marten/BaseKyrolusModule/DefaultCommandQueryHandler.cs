...
public sealed class DefaultCommandQueryHandler<TResponse, TModel, TKey>(
    ...
    : ICommandQueryHandler<TResponse, TModel, TKey>,
      IKyrolusMartenCommandQueryHandler<TResponse, TModel, TKey>
    ...
    where TKey : notnull, IEquatable<TKey>
{
    ...
    private readonly IKyrolusMartenApiConfig<TResponse>? martenConfig = config as IKyrolusMartenApiConfig<TResponse>;
    private readonly IKyrolusMartenAuthorizationProvider<TResponse> authorizationProvider =
        serviceProvider.GetService<IKyrolusMartenAuthorizationProvider<TResponse>>()
        ?? KyrolusNoopMartenAuthorizationProvider<TResponse>.Instance;
    ...
    public async Task<IResult> HandleGetByIdAsync(TKey id, string? includedProps = null, string? includeGraph = null, string? fields = null, bool? cacheable = null, bool? includeDeleted = null)
    {
        ...
        ApplyCacheable(query, cacheable);
        ApplyGetByIdQueryOptions(query, id, useStringIncludes ? includes : null, includeExpressions, includeGraphValue, asNoTracking: null, useSplitQuery: null, tenantId: endpointContext?.TenantId);
    ...
    public async Task<IResult> HandleGetByKeysAsync(string[]? keys, string? includedProps = null, string? includeGraph = null, string? fields = null, bool? cacheable = null, bool? includeDeleted = null)
    {
        ...
        var includeExpressions = BuildIncludeExpressions(includes, out var useStringIncludes);
        var query = martenConfig?.QueryByKeyValues ?? new GetByKeyValuesQuery<TResponse, TKey>(keyValues, cacheable ?? false);

        ApplyCacheable(query, cacheable);
        ApplyGetByKeyValuesQueryOptions(query, keyValues, useStringIncludes ? includes : null, includeExpressions, includeGraphValue, asNoTracking: null, useSplitQuery: null, tenantId: endpointContext?.TenantId);
        if (includeDeleted == true)
        {
            TrySetProperty(query, IncludeDeletedPropertyName, true);
        }
    ...
    public async Task<IResult> HandleRestoreAsync(TKey id, bool? cacheable = null)
    {
        ...
        var keyValues = BuildKeyValues(id);
        var command = martenConfig?.RestoreCommand ?? new RestoreByIdCommand<TResponse, TKey>(keyValues, cacheable ?? false);
        ApplyCacheable(command, cacheable);
        TrySetProperty(command, KeyValuesPropertyName, keyValues);
    ...
    public async Task<IResult> HandleGetAllPagedAsync(KyrolusMartenQueryParameters parameters, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IResult> HandleQueryPagedAsync(KyrolusMartenPagedQueryRequest request, CancellationToken cancellationToken = default)
    {
        var queryRequest = request.Request ?? (QueryRequest?)new QueryRequest();
        var authResult = await ResolveAuthorizationAsync(
    ...
    public async Task<IResult> HandleSeekAsync(KyrolusMartenSeekQueryParameters parameters, CancellationToken cancellationToken = default)
    {
        var requestedIncludes = SplitCsv(parameters.Includes);
        var requestedFields = SplitCsv(parameters.Fields);
        var authResult = await ResolveAuthorizationAsync(
    ...
    public async Task<IResult> HandleQuerySeekAsync(KyrolusMartenSeekQueryRequest request, CancellationToken cancellationToken = default)
    {
        QueryRequest queryRequest = request.Request ?? new QueryRequest();
        var authResult = await ResolveAuthorizationAsync(
    ...
    public async Task<IResult> HandleBulkDeleteAsync(KyrolusMartenBulkDeleteRequest request, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(request);
        var authResult = await ResolveAuthorizationAsync(
    ...
    public async Task<IResult> HandleBulkPatchAsync(IAsyncEnumerable<KyrolusMartenBulkPatchItem> items, bool? cacheable = null, CancellationToken cancellationToken = default)
    {
        var authResult = await ResolveAuthorizationAsync(
    ...
    private bool TryParseBulkPatchChunk(
        ...
        KyrolusMartenAuthorizationResult<TResponse> authResult,
    ...
    private bool TryParseBulkPatchItem(
        KyrolusMartenBulkPatchItem item,
        KyrolusMartenAuthorizationResult<TResponse> authResult,
    ...
    private static string[]? ResolveBulkPatchKeys(KyrolusMartenBulkPatchItem item)
    {
    ...
    private async ValueTask<KyrolusMartenAuthorizationResult<TResponse>> ResolveAuthorizationAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var context = new KyrolusMartenAuthorizationContext<TResponse>(
            endpoint,
    ...
    private IResult BuildAuthorizationError(KyrolusMartenAuthorizationResult<TResponse> authResult)
    ...
    {
        errorResult = null;
        if (martenConfig?.RequireTenant != true) return true;
    ...
    private bool TryBuildContextFilter(out Expression<Func<TResponse, bool>>? expression, out IResult? errorResult)
    {
        errorResult = null;
        expression = null;
        if (martenConfig is null) return true;

        if (!TryBuildPropertyFilter(martenConfig.TenantPropertyName, endpointContext?.TenantId, martenConfig.RequireTenant, "tenant", out var tenantFilter, out errorResult))
        {
            return false;
        }

        if (!TryBuildPropertyFilter(martenConfig.ScopePropertyName, endpointContext?.ScopeKey, required: false, "scope", out var scopeFilter, out errorResult))
        {
            return false;
        }
    ...
    private bool TryApplyContextValues(TResponse entity, out IResult? errorResult)
    {
        errorResult = null;
        if (martenConfig is null) return true;
        if (!TryApplyContextValue(entity, martenConfig.TenantPropertyName, endpointContext?.TenantId, martenConfig.RequireTenant, "tenant", out errorResult))
        {
            return false;
        }

        if (!TryApplyContextValue(entity, martenConfig.ScopePropertyName, endpointContext?.ScopeKey, required: false, "scope", out errorResult))
        {
            return false;
        }
    ...
    private bool TryRejectContextUpdates(IReadOnlyDictionary<string, object> updates, out IResult? errorResult)
    {
        errorResult = null;
        if (martenConfig is null) return true;
        if (!string.IsNullOrWhiteSpace(martenConfig.TenantPropertyName)
            && updates.Keys.Any(k => string.Equals(k, martenConfig.TenantPropertyName, StringComparison.OrdinalIgnoreCase)))
        {
            errorResult = BuildBadRequest("Tenant cannot be updated.");
            return false;
        }

        if (!string.IsNullOrWhiteSpace(martenConfig.ScopePropertyName)
            && updates.Keys.Any(k => string.Equals(k, martenConfig.ScopePropertyName, StringComparison.OrdinalIgnoreCase)))
        {
        ...
        out IResult? errorResult)
    {
        ...
        var allowlist = BuildAllowlist(MergeAllowlist(martenConfig?.AllowedPatchProperties, allowedPatchProperties));
        var strict = martenConfig?.StrictPatchValidation ?? false;
    ...
    private bool TryEnsureTenantMatch(TResponse entity, out IResult? errorResult)
    {
        ...
        if (!TryEnsureContextMatch(entity, martenConfig.TenantPropertyName, endpointContext?.TenantId, "tenant", out errorResult))
        {
            return false;
        }

        if (!TryEnsureContextMatch(entity, martenConfig.ScopePropertyName, endpointContext?.ScopeKey, "scope", out errorResult))
        {
            return false;
        }
    ...
    private bool TryEnsureRowAuthorization(TResponse entity, KyrolusMartenAuthorizationResult<TResponse> authResult, out IResult? errorResult)
    {
        errorResult = null;
    ...
    private async Task<TResponse?> LoadByKeysAsync(object?[] keyValues, bool includeDeleted, bool? cacheable)
    {
        var query = martenConfig?.QueryByKeyValues ?? new GetByKeyValuesQuery<TResponse, TKey>(keyValues, cacheable ?? false);
        ApplyCacheable(query, cacheable);
        ApplyGetByKeyValuesQueryOptions(query, keyValues, includeProperties: null, includeExpressions: null, includeGraph: null, asNoTracking: true, useSplitQuery: null, tenantId: endpointContext?.TenantId);
        if (includeDeleted)
        {
            TrySetProperty(query, IncludeDeletedPropertyName, true);
        }
    ...
    {
        errorResult = null;
        if (martenConfig?.EnableEtags != true) return true;
    ...
    {
        if (martenConfig?.EnableEtags != true) return (true, null);
    ...
    {
        result = null!;
        if (martenConfig?.EnableEtags != true) return false;
    ...
    private bool TryGetRowVersionProperty(out PropertyInfo rowVersionProperty)
    {
        ...
        if (string.IsNullOrWhiteSpace(martenConfig?.RowVersionPropertyName)) return false;
        var prop = typeof(TResponse).GetProperty(martenConfig.RowVersionPropertyName, BindingFlags.Public | BindingFlags.Instance | BindingFlags.IgnoreCase);
    ...
    {
        errorResult = null;
        var strict = martenConfig?.StrictFilterValidation ?? false;
        var caseInsensitive = martenConfig?.FilterCaseInsensitive ?? false;
        if (!FilterBuilder.TryBuildFilterExpression<TResponse>(filter, BuildAllowlist(martenConfig?.AllowedFilterProperties), strict, caseInsensitive, out expression, out var error))
        {
    ...
    private bool TryBuildFilter(IReadOnlyList<FilterClause>? clauses, out Expression<Func<TResponse, bool>>? expression, out IResult? errorResult)
    {
        ...
        var strict = martenConfig?.StrictFilterValidation ?? false;
        var caseInsensitive = martenConfig?.FilterCaseInsensitive ?? false;
        if (!FilterBuilder.TryBuildFilterExpression<TResponse>(clauses, BuildAllowlist(martenConfig?.AllowedFilterProperties), strict, caseInsensitive, out expression, out var error))
        {
            errorResult = BuildBadRequest(error ?? "Invalid filter.");
            return false;
        }
    ...
    {
        var strict = martenConfig?.StrictFilterValidation ?? false;
        orderExpr = OrderBuilder.BuildOrderBy<TResponse>(orderBy, BuildAllowlist(martenConfig?.AllowedOrderProperties), strict, out var error);
    ...
    {
        var strict = martenConfig?.StrictFilterValidation ?? false;
        orderExpr = OrderBuilder.BuildOrderBy<TResponse>(clauses, BuildAllowlist(martenConfig?.AllowedOrderProperties), strict, out var error);
        if (error is null) { errorResult = null; return true; }
        ...
        out IResult? errorResult)
    {
        ...
        var strict = martenConfig?.StrictIncludeValidation ?? false;
        var allowlist = BuildAllowlist(MergeAllowlist(martenConfig?.AllowedIncludeProperties, allowedIncludes));
        ...
        out IResult? errorResult)
    {
        ...
        var allowlist = BuildAllowlist(MergeAllowlist(martenConfig?.AllowedSelectProperties, allowedFields));
        var strict = martenConfig?.StrictSelectValidation ?? false;
        var viewModelType = ResolveViewModelType(endpoint);
        ...
        out IResult? errorResult)
    {
        ...
        var maxDepth = martenConfig?.MaxIncludeGraphDepth ?? 0;
        if (maxDepth <= 0)
        {
    ...
    {
        var defaultSize = martenConfig?.DefaultPageSize > 0 ? martenConfig.DefaultPageSize : 50;
        var maxSize = martenConfig?.MaxPageSize > 0 ? martenConfig.MaxPageSize : 200;
    ...
    {
        var chunk = martenConfig?.BulkChunkSize ?? 200;
        ...
        GetAllQueryOptions options)
    {
        if (query is GetAllQuery<TResponse> getAll)
        {
            ...
            getAll.TenantId = options.TenantId;
            return;
        }
        ...
        TrySetProperty(query, "TenantId", options.TenantId);
    }
        ...
        string? tenantId)
    {
        ...
        TrySetProperty(query, "TenantId", tenantId);
    }
    ...
    {
        errorResult = null;
        if (martenConfig?.SetCompositeKey is not null)
        {
            martenConfig.SetCompositeKey(entity, keyValues);
            return true;
        }

        var names = martenConfig?.CompositeKeyPropertyNames;
    ...
    private bool TryResolveSeekProperties(out IReadOnlyList<string> properties, out IResult? errorResult)
    {
        ...
        if (martenConfig?.CompositeKeyPropertyNames is { Count: > 0 } composite)
        {
            properties = composite;
            return true;
        }

        if (!string.IsNullOrWhiteSpace(martenConfig?.KeyPropertyName))
        {
            properties = [martenConfig.KeyPropertyName];
            return true;
        }
    ...
    private bool TryBuildKeyValues(IReadOnlyList<string>? keys, out object?[] keyValues, out IResult? errorResult)
    {
        ...
        if (martenConfig?.CompositeKeyParser is not null)
        {
            try
            {
                keyValues = martenConfig.CompositeKeyParser(normalized);
            }
            catch (Exception ex)
            {
        ...
        var keyTypes = martenConfig?.CompositeKeyTypes;
