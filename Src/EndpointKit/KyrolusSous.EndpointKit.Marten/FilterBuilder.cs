...
namespace KyrolusSous.EndpointKit.Marten;
...
public static class FilterBuilder
{
        ...
        out string? error)
    {
        ...
        if (!string.IsNullOrWhiteSpace(rawContent) && LooksLikeFilter(rawContent))
        {
            ...
            var ok = (bool)generic.Invoke(null, invokeArgs)!;
            if (!ok)
            {
                error = (string?)invokeArgs[5];
                return false;
            }

            var nested = invokeArgs[4] as LambdaExpression;
            if (nested is null)
            {
