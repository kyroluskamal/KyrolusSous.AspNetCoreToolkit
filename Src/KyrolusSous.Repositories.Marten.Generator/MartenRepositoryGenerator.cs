...
public sealed class MartenRepositoryGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateRepository(StringBuilder sb, RepositoryRequest request, IReadOnlyList<IncludeMapping> includeMappings, IReadOnlyList<IPropertySymbol> patchProperties, KeyPropertyInfo? keyProperty)
    {
        ...
        sb.AppendLine("    private Task? policyInitTask;");
        sb.AppendLine();
        ...
        sb.AppendLine("        policyProvider = services?.PolicyProvider;");
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        ...
        sb.AppendLine("    private async Task EnsurePolicyInitializedAsync(CancellationToken ct)");
        AppendOpenBrace(sb, Indent4);
        sb.AppendLine("        if (policyProvider is null) return;");
        sb.AppendLine();
        sb.AppendLine("        var initTask = policyInitTask;");
        sb.AppendLine("        if (initTask is null)");
        AppendOpenBrace(sb, Indent8);
        sb.AppendLine("            var newTask = InitializePolicyAsync(ct);");
        sb.AppendLine("            initTask = Interlocked.CompareExchange(ref policyInitTask, newTask, null) ?? newTask;");
        AppendCloseBrace(sb, Indent8);
        sb.AppendLine();
        sb.AppendLine("        await initTask.ConfigureAwait(false);");
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        sb.AppendLine("    private async Task InitializePolicyAsync(CancellationToken ct)");
        AppendOpenBrace(sb, Indent4);
        sb.AppendLine("        var context = new KyrolusMartenRepositoryPolicyContext(");
        sb.AppendLine("            typeof(TEntity),");
        sb.AppendLine("            typeof(TEntity).Name,");
        sb.AppendLine("            typeof(" + sessionType + "),");
        sb.AppendLine("            ResolveCacheScope(null),");
        sb.AppendLine("            cacheKeyContext?.TenantId);");
        sb.AppendLine();
        sb.AppendLine("        var dynamicDeps = await policyProvider!.GetPolicyAsync(context, ct).ConfigureAwait(false);");
        sb.AppendLine("        if (dynamicDeps is null) return;");
        sb.AppendLine();
        sb.AppendLine("        ApplyPolicy(dynamicDeps);");
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        sb.AppendLine("    private void ApplyPolicy(KyrolusMartenRepositoryDependencies deps)");
        AppendOpenBrace(sb, Indent4);
        sb.AppendLine("        if (deps.Observer is not null) observer = deps.Observer;");
        sb.AppendLine("        if (deps.Authorization is not null) authorization = deps.Authorization;");
        sb.AppendLine("        if (deps.Validation is not null) validation = deps.Validation;");
        sb.AppendLine("        if (deps.SoftDeletePolicy is not null) softDeletePolicy = deps.SoftDeletePolicy;");
        sb.AppendLine("        if (deps.CacheProvider is not null) cacheProvider = deps.CacheProvider;");
        sb.AppendLine("        if (deps.CacheKeyContext is not null) cacheKeyContext = deps.CacheKeyContext;");
        sb.AppendLine("        if (deps.CachePolicyProvider is not null) cachePolicyProvider = deps.CachePolicyProvider;");
        sb.AppendLine("        if (deps.CachePolicy is not null) cachePolicy = deps.CachePolicy;");
        sb.AppendLine("        if (deps.ResiliencePolicy is not null) resilience = deps.ResiliencePolicy;");
        sb.AppendLine("        if (deps.Tracing is not null) tracing = deps.Tracing;");
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        sb.AppendLine("    private async Task<T> ExecAsync<T>(string operation, object? payload, Func<Task<T>> action, CancellationToken ct)");
        AppendOpenBrace(sb, Indent4);
        sb.AppendLine("        await EnsurePolicyInitializedAsync(ct).ConfigureAwait(false);");
        ...
        if (request.EnableSoftDelete)
        {
            ...
            sb.AppendLine("            if (cacheProvider is not null && isCacheable)");
            AppendOpenBrace(sb, Indent12);
            sb.AppendLine("                var resolvedPolicy = await ResolveCachePolicyAsync(\"GetAllIncludingDeletedAsync\", opts.TenantId, cancellationToken).ConfigureAwait(false);");
            sb.AppendLine("                if (IsCacheEnabled(resolvedPolicy))");
            AppendOpenBrace(sb, Indent12 + 4);
            sb.AppendLine("                    var cacheKey = BuildCacheAllKey(opts.TenantId, resolvedPolicy.KeySuffix) + \":incdel=1\";");
            sb.AppendLine("                    var cacheOptions = BuildCacheEntryOptions(resolvedPolicy, opts.TenantId);");
            sb.AppendLine("                    return await cacheProvider.GetOrCreateAsync<List<TEntity>>(cacheKey, async ct =>");
            AppendOpenBrace(sb, Indent12 + 8);
            sb.AppendLine("                        var query = BuildQuery(opts, out var activeSession);");
            ...
            AppendIndentedLine(sb, Indent8, LineDefaultOptions);
            sb.AppendLine("        var hasUserFilter = opts.Filter is not null;");
            ...
            sb.AppendLine("            var hasIncludes = includes.Count > 0;");
            sb.AppendLine("            var isCacheable = !hasIncludes");
            sb.AppendLine("                && !hasUserFilter");
            sb.AppendLine("                && opts.Specification is null");
            sb.AppendLine("                && opts.OrderBy is null");
            sb.AppendLine("                && opts.ConfigureQuery is null;");
            sb.AppendLine("            if (cacheProvider is not null && isCacheable)");
            AppendOpenBrace(sb, Indent12);
            sb.AppendLine("                var resolvedPolicy = await ResolveCachePolicyAsync(\"GetDeletedOnlyAsync\", opts.TenantId, cancellationToken).ConfigureAwait(false);");
            sb.AppendLine("                if (IsCacheEnabled(resolvedPolicy))");
            AppendOpenBrace(sb, Indent12 + 4);
            sb.AppendLine("                    var cacheKey = BuildCacheAllKey(opts.TenantId, resolvedPolicy.KeySuffix) + \":incdel=1:delonly=1\";");
            sb.AppendLine("                    var cacheOptions = BuildCacheEntryOptions(resolvedPolicy, opts.TenantId);");
            sb.AppendLine("                    return await cacheProvider.GetOrCreateAsync<List<TEntity>>(cacheKey, async ct =>");
            AppendOpenBrace(sb, Indent12 + 8);
            sb.AppendLine("                        var query = BuildQuery(opts, out var activeSession);");
    ...
    private static void AppendCacheHelpers(StringBuilder sb, RepositoryRequest request, KeyPropertyInfo? keyInfo, string keyType)
    {
        ...
        sb.AppendLine("        var extraKeys = MergeInvalidationEntries(basePolicy.ExtraInvalidationKeys, overridePolicy.ExtraInvalidationKeys);");
        sb.AppendLine("        var extraPatterns = MergeInvalidationEntries(basePolicy.ExtraInvalidationKeyPatterns, overridePolicy.ExtraInvalidationKeyPatterns);");
        ...
        sb.AppendLine("            ExtraInvalidationKeys: extraKeys,");
        sb.AppendLine("            ExtraInvalidationKeyPatterns: extraPatterns);");
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        sb.AppendLine("    private static IReadOnlyCollection<string>? MergeInvalidationEntries(");
        sb.AppendLine("        IReadOnlyCollection<string>? baseEntries,");
        sb.AppendLine("        IReadOnlyCollection<string>? overrideEntries)");
        AppendOpenBrace(sb, Indent4);
        sb.AppendLine("        if (baseEntries is null || baseEntries.Count == 0) return overrideEntries;");
        sb.AppendLine("        if (overrideEntries is null || overrideEntries.Count == 0) return baseEntries;");
        sb.AppendLine("        return baseEntries.Concat(overrideEntries).Distinct(StringComparer.Ordinal).ToArray();");
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        ...
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        sb.AppendLine("    private IEnumerable<string> ExpandInvalidationTemplates(");
        sb.AppendLine("        IReadOnlyCollection<string>? templates,");
        sb.AppendLine("        TKey? id,");
        sb.AppendLine("        bool hasId,");
        sb.AppendLine("        string? tenantId,");
        sb.AppendLine("        KyrolusCachePolicy policy)");
        AppendOpenBrace(sb, Indent4);
        sb.AppendLine("        if (templates is null || templates.Count == 0) yield break;");
        sb.AppendLine();
        sb.AppendLine("        var scope = ResolveCacheScope(tenantId);");
        sb.AppendLine("        var resolvedTenant = ResolveTenantId(tenantId);");
        sb.AppendLine("        var allKey = BuildCacheAllKey(tenantId, policy.KeySuffix);");
        sb.AppendLine("        var idKey = hasId ? BuildCacheKey(tenantId, id!, policy.KeySuffix) : null;");
        sb.AppendLine("        foreach (var template in templates)");
        AppendOpenBrace(sb, Indent8);
        sb.AppendLine("            if (string.IsNullOrWhiteSpace(template)) continue;");
        sb.AppendLine("            if (!hasId && template.Contains(\"{id}\", StringComparison.Ordinal)) continue;");
        sb.AppendLine();
        sb.AppendLine("            var expanded = template");
        sb.AppendLine("                .Replace(\"{entity}\", typeof(TEntity).Name, StringComparison.Ordinal)");
        sb.AppendLine("                .Replace(\"{tenant}\", resolvedTenant ?? string.Empty, StringComparison.Ordinal)");
        sb.AppendLine("                .Replace(\"{scope}\", scope ?? string.Empty, StringComparison.Ordinal)");
        sb.AppendLine("                .Replace(\"{policy}\", policy.KeySuffix ?? string.Empty, StringComparison.Ordinal)");
        sb.AppendLine("                .Replace(\"{all}\", allKey, StringComparison.Ordinal);");
        sb.AppendLine("            expanded = expanded.Replace(\"{id}\", idKey ?? string.Empty, StringComparison.Ordinal);");
        sb.AppendLine("            if (!string.IsNullOrWhiteSpace(expanded))");
        AppendOpenBrace(sb, Indent12);
        sb.AppendLine("                yield return expanded;");
        AppendCloseBrace(sb, Indent12);
        AppendCloseBrace(sb, Indent8);
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        sb.AppendLine("    private async Task RemoveExtraInvalidationKeysAsync(");
        sb.AppendLine("        TKey? id,");
        sb.AppendLine("        bool hasId,");
        sb.AppendLine("        string? tenantId,");
        sb.AppendLine("        KyrolusCachePolicy policy,");
        ...
        sb.AppendLine("        foreach (var key in ExpandInvalidationTemplates(policy.ExtraInvalidationKeys, id, hasId, tenantId, policy))");
        AppendOpenBrace(sb, Indent8);
        sb.AppendLine("            await cacheProvider.RemoveAsync(key, cancellationToken).ConfigureAwait(false);");
        AppendCloseBrace(sb, Indent8);
        sb.AppendLine("        foreach (var pattern in ExpandInvalidationTemplates(policy.ExtraInvalidationKeyPatterns, id, hasId, tenantId, policy))");
        AppendOpenBrace(sb, Indent8);
        sb.AppendLine("            await cacheProvider.RemoveKeysByPatternAsync(pattern, cancellationToken).ConfigureAwait(false);");
        AppendCloseBrace(sb, Indent8);
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        ...
        AppendCloseBrace(sb, Indent8);
        sb.AppendLine("        await RemoveExtraInvalidationKeysAsync(id, true, tenantId, resolvedPolicy, cancellationToken).ConfigureAwait(false);");
        ...
        sb.AppendLine("        if (SoftDeleteEnabled)");
        AppendOpenBrace(sb, Indent8);
        sb.AppendLine("            var allDeletedKey = allKey + \":incdel=1\";");
        sb.AppendLine("            await cacheProvider.RemoveAsync(allDeletedKey, cancellationToken).ConfigureAwait(false);");
        sb.AppendLine("            var deletedOnlyKey = allKey + \":incdel=1:delonly=1\";");
        sb.AppendLine("            await cacheProvider.RemoveAsync(deletedOnlyKey, cancellationToken).ConfigureAwait(false);");
        AppendCloseBrace(sb, Indent8);
        sb.AppendLine("        await RemoveExtraInvalidationKeysAsync(default, false, tenantId, resolvedPolicy, cancellationToken).ConfigureAwait(false);");
        AppendCloseBrace(sb, Indent4);
        sb.AppendLine();
        ...
        else
        {
            ...
            AppendOpenBrace(sb, Indent8);
            sb.AppendLine("            var allDeletedKey = allKey + \":incdel=1\";");
            sb.AppendLine("            tasks.Add(cacheProvider.RemoveAsync(allDeletedKey, cancellationToken));");
            sb.AppendLine("            var deletedOnlyKey = allKey + \":incdel=1:delonly=1\";");
            sb.AppendLine("            tasks.Add(cacheProvider.RemoveAsync(deletedOnlyKey, cancellationToken));");
            AppendCloseBrace(sb, Indent8);
            sb.AppendLine("        tasks.Add(RemoveExtraInvalidationKeysAsync(default, false, tenantId, resolvedPolicy, cancellationToken));");
            sb.AppendLine("        foreach (var entity in entities)");
            AppendOpenBrace(sb, Indent8);
            ...
            AppendCloseBrace(sb, Indent12);
            sb.AppendLine("            tasks.Add(RemoveExtraInvalidationKeysAsync(id, true, tenantId, resolvedPolicy, cancellationToken));");
            AppendCloseBrace(sb, Indent8);
