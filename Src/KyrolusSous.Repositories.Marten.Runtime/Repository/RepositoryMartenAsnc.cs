    ...
    where TKey : IEquatable<TKey>
{
    ...
    {
        var includeProps = includeExpressions?.Length > 0 ? includeExpressions.Select(GetPropertyName).ToList() : null;
        return GetAllAsync(filter, orderBy, includeProps, cancellationToken, asNoTracking);
    }

    public async Task<TEntity?> GetByIdAsync(TKey id, List<string>? includeProperties = null, CancellationToken cancellationToken = default, bool asNoTracking = true)
    {
    ...
    public async Task<TEntity?> GetByIdAsync(object?[] keyValues, List<string>? includeProperties = null, CancellationToken cancellationToken = default, bool asNoTracking = true)
    {
        ...
        return await GetByIdAsync((TKey)Convert.ChangeType(key, typeof(TKey)), includeProperties, cancellationToken);
    }
    ...
    {
        var includeProps = includeExpressions?.Length > 0 ? includeExpressions.Select(GetPropertyName).ToList() : null;
        return GetByIdAsync(id, includeProps, cancellationToken, asNoTracking);
    }
    ...
    {
        var includeProps = includeExpressions?.Length > 0 ? includeExpressions.Select(GetPropertyName).ToList() : null;
        return GetByIdAsync(keyValues, includeProps, cancellationToken, asNoTracking);
    }
    ...
    private static string GetPropertyName(Expression<Func<TEntity, object?>> expression)
    {
        ...
        throw new ArgumentException("Include expression must be a member access", nameof(expression));
    }
