...
public partial class GetAllIncludingDeletedAsync_Filter
{
    ...
    [Theory(DisplayName = "GetAllIncludingDeletedAsync supports filter operators for single-key entities")]
    [MemberData(nameof(SingleKeyOperatorCases))]
    public Task GetAllIncludingDeletedAsync_SingleKey_Operators_Work(string caseId)
    {
    ...
    [Theory(DisplayName = "GetAllIncludingDeletedAsync supports filter operators for composite-key entities")]
    [MemberData(nameof(CompositeKeyOperatorCases))]
    public Task GetAllIncludingDeletedAsync_CompositeKey_Operators_Work(string caseId)
    {
        caseId.ShouldNotBeNullOrWhiteSpace();
    ...
    private static IReadOnlyDictionary<string, OperatorSpec<Review>> BuildCompositeKeyOperatorSpecs()
    {
        ...
        AddEqCases(data, "isdeleted", nameof(Review.IsDeleted), "false", 2,
            reviews => reviews.All(r => !r.IsDeleted ).ShouldBeTrue());
        AddNeqCases(data, "isdeleted", nameof(Review.IsDeleted), "true", 2,
            reviews => reviews.All(r => !r.IsDeleted).ShouldBeTrue());
