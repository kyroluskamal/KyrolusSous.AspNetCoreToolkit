...
public partial class RestoreAsyncTests
{
    ...
    protected Task<(HttpResponseMessage Response, string Content)> PostSingleRestoreAsync<TEntity>(Guid id)
    {
        var route = $"/api/{typeof(TEntity).Name.ToLowerInvariant()}/{id}/restore";
        return PostRawAsync(route);
    }

    protected Task<(HttpResponseMessage Response, string Content)> PostCompositeRestoreAsync<TEntity>(object?[] keys)
    {
        var keysQuery = string.Join("&", keys.Select(x => $"keys={Uri.EscapeDataString(x?.ToString() ?? string.Empty)}"));
        var route = $"/api/{typeof(TEntity).Name.ToLowerInvariant()}/restore/by-id?{keysQuery}";
        return PostRawAsync(route);
    }
        ...
        string? sku = null)
    {
        ...
        return new Product
        {
            ...
            Name = name ?? $"RestoreProduct-{token}",
            Sku = sku ?? $"RST-{token}",
            Price = 60m,
