...
public partial class StreamAsyncTests
{
    ...
    protected static async Task<List<TEntity>> CollectAsync<TEntity>(IAsyncEnumerable<TEntity> stream)
    {
    ...
    protected async Task<(HttpResponseMessage Response, List<TEntity>? Items, string Content)> GetStreamAsync<TEntity>(QueryRequest? request = null)
    {
        var route = $"/api/{typeof(TEntity).Name.ToLowerInvariant()}/stream";
        ...
        string? sku = null)
    {
        ...
        return new Product
        {
            ...
            Name = name ?? $"StreamProduct-{token}",
            Sku = sku ?? $"STM-{token}",
            Price = 66m,
            AddedIn = new DateOnly(2026, 11, 1),
