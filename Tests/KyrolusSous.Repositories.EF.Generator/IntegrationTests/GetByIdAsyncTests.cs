...
// {
//     [Fact(DisplayName = "GetAllAsync should return entity without Include Properties or filters")]
//     public async Task GetAllAsync_NoIncludeNoFilterNoOrder_ReturnsAllEntities()
//     {
//         var (response, reviews, _) = await ArrangeAndActUseingHttpForGetByIdAsync<Review>();
//         // Assert
...
//     {
//         var (response, products, _) = await ArrangeAndActUseingHttpForGetByIdAsync<Product>(new QueryRequest(OrderBy: [new OrderClause("StockQuantity")]));
//         // Assert
//         response.EnsureSuccessStatusCode();
...
//     {
//         var (response, products, _) = await ArrangeAndActUseingHttpForGetByIdAsync<Product>(new QueryRequest(OrderBy: [new OrderClause("StockQuantity", true)]));
//         // Assert
//         response.EnsureSuccessStatusCode();
//         products.ShouldNotBeNull();
...
//     {
//         var (_, products, _) = await ArrangeAndActUseingHttpForGetByIdAsync<Product>(new QueryRequest(OrderBy: [
...
//     {
//         var (response, products, _) = await ArrangeAndActUseingHttpForGetByIdAsync<Product>(new QueryRequest(
...
//     {
//         var (_, products, _) = await ArrangeAndActUseingHttpForGetByIdAsync<Product>(new QueryRequest(
...
//     {
//         var (_, reviews, _) = await ArrangeAndActUseingHttpForGetByIdAsync<Review>(
...
//     {
//         var (_, products, _) = await ArrangeAndActUseingHttpForGetByIdAsync<Product>(
